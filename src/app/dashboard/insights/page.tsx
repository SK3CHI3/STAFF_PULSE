...
export default function InsightsPage() {
  const { user, profile: authProfile, loading: profileLoading } = useAuth()
  ...
  useEffect(() => {
    if (authProfile && authProfile.organization) {
      ...
      // Debug: log loaded profile from useAuth
      if (typeof window !== 'undefined') {
        console.log('[Insights] Loaded profile from useAuth:', authProfile)
      }
    } else {
      ...
      if (cachedProfile) {
        try {
          ...
          // Debug: log loaded profile from localStorage
          if (typeof window !== 'undefined') {
            console.log('[Insights] Loaded profile from localStorage:', parsed)
            console.log('[Insights] Parsed organization:', org)
          }
        } catch (e) {
          // Ignore invalid cache
          if (typeof window !== 'undefined') {
            console.error('[Insights] Failed to parse profile from localStorage:', e)
          }
        }
      } else {
        ...
        setProfile(null)
        if (typeof window !== 'undefined') {
          console.warn('[Insights] No profile found in useAuth or localStorage')
        }
      }
    }
  }, [authProfile])
  ...
  // Generate new insights via API route
  const generateNewInsights = async (auto = false) => {
    try {
      ...
      if (result.success) {
        ...
        else setError('Failed to generate insights: Unknown error')
      }
    } catch (err: any) {
      if (!auto) setError('Error generating insights: ' + (err.message || err))
      if (typeof window !== 'undefined') {
  ...
  // If profile loaded but org is missing, show error and log out button
  if (!profile || !profile.organization?.id) {
    return (
      <div className="bg-gray-50 min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="text-red-600 text-lg font-medium">No organization found</div>
          <p className="text-gray-600 mt-2">Please log in again or contact support.</p>
          <button
